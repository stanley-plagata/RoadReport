	...
	<script>
		...
		function initMap() {
			...
			ajaxGet();
				...
				function ajaxGet() {
					$.ajax({
						cache : false,
						type : "GET",
						url : '${pageContext.request.contextPath}/testGet',
							...
							for (var i = 0; i < hazards.length; i++) {
								...
								google.maps.event.addListener(marker, 'click', (function (marker, i) {
						            return function () {
						            	...
						            	'<br><input type="button" value="View more details" onClick="goToHazardDetails(\'' + address + '\' , \'' + reportID + '\' , \'' + hazardID + '\' )" />';
			...
			function ajaxPost() {
				...
				$.ajax({
					type : "POST",
					url : '${pageContext.request.contextPath}/testPost',
					data : {
		...
		function goToHazardDetails(address, reportID, hazardID)	{
			//alert(address + " AND " + reportID + " AND " + hazardID);
